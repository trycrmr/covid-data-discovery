...
const app = express();
const port = 3000;
const UNCOMPRESSED_DATA_SIZE = 156099104;
...
app.get("/plotly-1.53.0.min.js.gz", (req, res) => {
  // Freezing plotly version so the dependency doesn't change underfoot and this app can control the headers
  res.setHeader("Content-Encoding", "gzip");
  res.setHeader("Transfer-Encoding", "gzip");
...
app.listen(port, () =>
  console.log(
    `coviddata.website server running on port ${port}; Give EVERYONE EVERYWHERE insight into the latest COVID-19 data.`
  )
);
